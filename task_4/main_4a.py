...
if __name__ == '__main__':
    ...
    single_point = test_set.dataset[2][0]
    ...
    model = train(enc, dec, device, single_point_loader, ELBOLoss(), optimizer, num_lags=2)
    ...
    with torch.no_grad():
        ...
        axarr[1].imshow(x_reconstr.cpu().permute(1, 2, 0).numpy())  # Reconstruction in new way
        out = enc_trained(single_point.unsqueeze(0))
        embeddings = out[1].cpu()
        output = dec(out[0])[0].cpu()
        output = output.squeeze(0)
        axarr[2].imshow(output.cpu().permute(1, 2, 0).numpy())  # Reconstruction as in 1D
